...
class BuildingStrategy extends BaseBuildingStrategy
{
  function homes_for_full_employment() {
    ...
    $incoming_jobs += 20 * (array_sum($this->incoming_acres) - $this->incoming_acres['land_plain']);
    $total_jobs = $jobs + $incoming_jobs;

    $incoming_peasants = ($this->incoming_buildings['building_home'] * 15 + array_sum($this->incoming_acres) * 15 +  $this->incoming_acres['land_plain'] * 15) * $this->populationCalculator->getMaxPopulationMultiplier($this->dominion);
  ...
  public function get_land_types_to_explore($dominion, $tick, $max_afford) {
    ...
    $wanted_dm = $this->build_to_max_nr('building_diamond_mine', 800, 'cavern', $capacity);
    $acres_to_explore['land_cavern'] += $wanted_dm;
    $capacity = $max_afford - array_sum($acres_to_explore);
