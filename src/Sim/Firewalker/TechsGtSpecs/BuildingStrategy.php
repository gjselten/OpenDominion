...
class BuildingStrategy extends BaseBuildingStrategy
{
  public function get_land_types_to_explore($dominion, $tick, $max_afford) {
    ...
    $acres_to_explore['land_plain'] = $this->to_explore_by_percentage('building_farm', $new_acres, 0.068, 'plain', $capacity);
    ...
    $acres_to_explore['land_cavern'] += $this->to_explore_by_percentage('building_home', $new_acres, 0.22, 'cavern', $capacity);
    $capacity = $max_afford - array_sum($acres_to_explore);
  ...
  public function get_buildings_to_build($dominion, $tick, $max_afford) {
    ...
    $farms_needed = round($this->current_acres * 0.068 - $this->dominion->building_farm - $this->incoming_buildings['building_farm']);
    if($farms_needed > 0) {
